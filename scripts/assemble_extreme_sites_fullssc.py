...
FULL SSC VERSION: uses all_ssc_samples.parquet (5,078 samples, full range)
instead of the >1000-only hotspot file.

Sources:
1. Continuous FNU turbidity: data/continuous/USGS_{site_no}/63680/
2. Discrete SSC: data/raw/extreme_ssc_hotspots/all_ssc_samples.parquet  (ALL concentrations)
...
def load_hotspot_ssc(site_id: str) -> pd.DataFrame:
    """Load SSC from all_ssc_samples.parquet (full range, already UTC) for one site.

    all_ssc_samples.parquet has columns:
      site_id, datetime (UTC tz-aware), ssc_mg_L, activity_id, collection_method,
      medium, detection_condition, detection_limit, units
    """
    all_ssc = pd.read_parquet(DATA_DIR / "raw" / "extreme_ssc_hotspots" / "all_ssc_samples.parquet")
    sub = all_ssc[all_ssc["site_id"] == site_id].copy()
    if sub.empty:
        return pd.DataFrame()

    # datetime is already UTC tz-aware — just ensure it's parsed correctly
    sub["datetime"] = pd.to_datetime(sub["datetime"], utc=True)
    sub = sub.dropna(subset=["datetime", "ssc_mg_L"])
...
def assign_split(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    Explicit assignments (from task spec):
    - vault:   USGS-09153270 (Cement Creek CO) — richest extreme data
    - holdout: USGS-06902000 (Gasconade River MO)  [east, large river]
               + 2-3 more from different states for geographic diversity:
                 USGS-14123500 (Skokomish WA)     [Pacific NW, highest single sample]
                 USGS-06174500 (Yellowstone MT)   [Northern Rockies]
                 USGS-07170000 (Fall River KS)    [Great Plains]
    - training: all remaining

    Target: ~70-75% training, ~20-25% holdout, ~5% vault by site count.
    ScienceBase sites go to training (pre-paired, no anchor-site risk).
    """
    dataset = dataset.copy()
    dataset["suggested_role"] = "training"  # default

    VAULT_SITES = [
        "USGS-09153270",  # Cement Creek CO — richest extreme data (163 samples >= 5000 mg/L)
    ]
    HOLDOUT_SITES = [
        "USGS-06902000",  # Gasconade River MO — eastern US diversity
        "USGS-14123500",  # Skokomish WA — Pacific NW, max SSC 733,000 mg/L
        "USGS-06174500",  # Yellowstone MT — Northern Rockies
        "USGS-07170000",  # Fall River KS — Great Plains
    ]
    ...
    cont_sites = dataset[dataset["turb_source"] == "continuous"]["site_id"].unique().tolist()
    ...
    logger.info(f"Split: {len(training_sites)} training sites (continuous)")
